// lightning_brakedown.cpp — Lightning PCS with Brakedown inner code
//
// Compile:
...
// Lightning Table 3 params (optimized for Brakedown base code)
struct Params {
    double alpha;
    int degree;
};
...
const int poly_var = 24;
const double delta = 0.004;

const double alpha   = table[delta].alpha;
const int    degree  = table[delta].degree;
...
const int t = compute_t(delta);

int get_best_col_num(int N, int t) {
    int best = 1;
    long long best_ps = 1LL << 40;
    for (int col = 1; col <= N - 1; col++) {
        int row = N - col;
        long long size = (1LL << row) * t + 3 * (1LL << col);
        if (size < best_ps) {
            best_ps = size;
            best = col;
        }
...
const int COLUMN_NUMBER = 1LL << get_best_col_num(poly_var, t);
...
struct Random_matrix {
    int left_size, right_size, degree;
    vector<vector<int>> edges;

    Random_matrix(int lsize, int rsize, int deg)
        : left_size(lsize), right_size(rsize), degree(deg)
    {
    ...
    // Single vector: out[v] += in[u] for each edge (u,v)
    void aggregate_Fr(F* in, F* out) const {
        for (int u = 0; u < left_size; u++) {
    ...
    // Batch: in is left_size × ROW_NUMBER (column-major), out is right_size × ROW_NUMBER
    void aggregate_batch_left_major(const uint64_t* in, F* out) const {
        vector<Fr> acc((size_t)right_size * ROW_NUMBER);
...
int main() 
{
    ...
    vector<F> compressed((size_t)right_size * ROW_NUMBER); // output of LDPC aggregate
    vector<F> codeword((size_t)second_len * ROW_NUMBER);   // inner code output
    vector<F> rsout(second_len);                            // temp for one row's encoding

    // Random messages
    mt19937_64 rng(12345);
    uniform_int_distribution<uint64_t> dist(0, (long long)1e10);
    for (size_t i = 0; i < (size_t)COLUMN_NUMBER * ROW_NUMBER; i++)
        in[i] = dist(rng);

    // LDPC matrix (binary D-row-regular)
    Random_matrix g(COLUMN_NUMBER, right_size, degree);

    // ---- Commit ----
    auto t0 = steady_clock::now();

    // Step 1: LDPC aggregate  x·A → compressed
    ...
    for (int i = 0; i < COLUMN_NUMBER; i++)
        mroot[i] = sha256(reinterpret_cast<const uint8_t*>(&in[(size_t)i * ROW_NUMBER]),
    ...
    Hash ROOT = sha256(reinterpret_cast<const uint8_t*>(mroot),
    ...
    memset(yr, 0, sizeof(F) * codeword_len);
    memset(y1, 0, sizeof(F) * codeword_len);
    ...
    // Verifier recomputes: LDPC aggregate yr, then Brakedown encode
    vector<F> tmp_v(right_size, F(0));
    g.aggregate_Fr(yr, tmp_v.data());
    // Encode the aggregated result with Brakedown inner code
    vector<F> inner_cw(second_len);
    inner_code.encode(tmp_v.data(), inner_cw.data());
    for (int i = 0; i < second_len; i++)
        yr[COLUMN_NUMBER + i] = inner_cw[i];

    // Spot-check t columns
    ...
    long long PS = (long long)t * ROW_NUMBER + 2 * COLUMN_NUMBER + codeword_len;
